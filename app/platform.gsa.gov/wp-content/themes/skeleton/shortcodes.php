...
function randomize_theme_page_func($atts, $content = null)
{
	extract(shortcode_atts(array(
		 'show' => '',
		 'total' => '',
    ...
    $rand_themes = non_repeat(1,$total,$show);
    sort($rand_themes);
    $return_string = "<style type=\"text/css\">\n";
    
    $return_string .= ".sites_theme {display:none;}\n.sites_theme img{max-width:320px;}\n#theme_links{margin-left:30px;list-style-type:disc;}\n#theme_links li {line-height:12px;}\n#theme_links li a {line-height:12px;text-decoration:none;}\n";
    foreach($rand_themes as $key => $value)
    {
    	$this_css = ($key%2 == 0) ? "margin: 0px 15px 10px 0px;float:left;":"margin: 0px 0px 10px 15px;float:right;";
    	$return_string .= '#'.$base_id.$value.' {display:block;}'."\n".'#'.$base_id.$value.' img{'.$this_css.'}'."\n";
    	$return_string .= '#theme_link'.$value.' {display:none;}'."\n";
    }

    $return_string .= "</style>";
    echo $return_string;
}
add_shortcode('randomize-theme-page', 'randomize_theme_page_func');
...
/*
function insert_cf7_validation_js()
{
	echo'<script type="text/javascript">
	jQuery(document).ready(function( $ ) {
	    $(".wpcf7-submit").click(function(e) {
	        email_in = document.getElementsByName("your-email")[0].value;
	        if(email_in.substr(email_in.length-4) != \'.gov\' && email_in.substr(email_in.length-4) != \'.mil\')
	        {
	            e.preventDefault();
	            alert("Must have a valid .gov or .mil email");
	        }
	    });
	});
	</script>';
}
add_shortcode('cf7-validation', 'insert_cf7_validation_js');
*/