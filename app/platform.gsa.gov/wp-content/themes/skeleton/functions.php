...
<li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
			<div id="comment-<?php comment_ID(); ?>" class="single-comment clearfix">
				<div class="comment-author vcard"> <?php echo get_avatar($comment,$size='64',$default='mm' ); ?></div>
...
if(is_admin())
{
	add_action('admin_menu', 'sites_mass_email_admin_menu');

	function sites_mass_email_admin_menu() {
	add_options_page('Mass Emailer Plugin', 'Mass Site Emailer', 'administrator',
	'sites_mass_email_plugin', 'sites_mass_emailer');
	}

	function sites_mass_emailer() {
	global $wp_roles;
	$status_message = "";
	$all_roles = $wp_roles->roles;
	if($_POST)
	{
		$from_name = $_POST['name_from'] == "" ? "Sites.Usa.gov" : $_POST['name_from'];
		$from_email = $_POST['email_from'] == "" ? "sitessupport@gsa.gov" : $_POST['email_from'];
		$email_subject = $_POST['email_subject'];
		$email_message = $_POST['email_message'];
		$recipients_in = $_POST['recipients'];
		...
		else
		{
			if($recipients_in == "")
			{
				echo "<font color=\"red\">Message FAILED.</font>  Reason: No recipents";
				$status_message .= "<font color=\"red\">Message FAILED.</font>  Reason: No recipents";
			}
			else
			{
				$all_recipients = explode(",",$recipients_in);
				foreach($all_recipients as $this_recipient)
				{
					$status_message .= trim($this_recipient);
					...
					{
						if(wp_mail(trim($this_recipient),$email_subject,$email_message,$message_headers))
	...
	?>
	<tr>
	<td width="200">Recipent(s):</td>
	<td width="510"><textarea name="recipients" id="recipients" value="" rows="5" cols="51"></textarea>
	</tr>
	<tr valign="top">
	...
	<td width="510"><input type="text" name="name_from" id="name_from" value="Sites.Usa.Gov" size="50">
	</tr>
	<tr>
	<td width="200">From Email:</td>
	<td width="510"><input type="text" name="email_from" id="email_from" value="sitessupport@gsa.gov" size="50">
	</tr>
	<tr>
