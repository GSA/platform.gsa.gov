...
<div id="event-rsvp-options" style="<?php echo ($EM_Event->event_rsvp) ? '':'display:none;' ?>">
	...
	if( get_option('dbem_bookings_tickets_single') && count($EM_Tickets->tickets) == 1 ){
		...
		<p class="text-info"><i class="fa fa-info"></i>  <?php esc_html_e('You can have single or multiple tickets, where certain tickets become availalble under certain conditions, e.g. early bookings, group discounts, maximum bookings per ticket, etc.', 'dbem'); ?> <?php esc_html_e('Basic HTML is allowed in ticket labels and descriptions.','dbem'); ?></p>					
	...
	<div id="em-booking-options form-group">
	...
	<p>
		...
		<input type="text" name="event_spaces" class="form-control" value="<?php if( $EM_Event->event_spaces > 0 ){ echo $EM_Event->event_spaces; } ?>" /><div class="text-info"><i class="fa fa-info"></i>  <?php esc_html_e('Individual tickets with remaining spaces will not be available if total booking spaces reach this limit. Leave blank for no limit.','dbem'); ?></div>
	</p>
	<p>
		<label><?php esc_html_e('Maximum Spaces Per Booking','dbem'); ?></label>
		<input class="form-control" type="text" name="event_rsvp_spaces" value="<?php if( $EM_Event->event_rsvp_spaces > 0 ){ echo $EM_Event->event_rsvp_spaces; } ?>" /><div class="text-info"><i class="fa fa-info"></i> <?php esc_html_e('If set, the total number of spaces for a single booking to this event cannot exceed this amount.','dbem'); ?><?php esc_html_e('Leave blank for no limit.','dbem'); ?></div>
	</p>
	<p>
		<label><?php esc_html_e('Booking Cut-Off Date','dbem'); ?></label>
		<span class="em-booking-date-normal">
			<span class="em-date-single">
				<input id="em-bookings-date-loc" class="em-date-input-loc form-control" type="text" />
				<input id="em-bookings-date" class="em-date-input form-control" type="hidden" name="event_rsvp_date" value="<?php echo $EM_Event->event_rsvp_date; ?>" />
			</span>
		</span>
		<span class="em-booking-date-recurring">
			<input class="form-control" type="text" name="recurrence_rsvp_days" size="3" value="<?php if( !empty($EM_Event->recurrence_rsvp_days) ) echo absint($EM_Event->recurrence_rsvp_days); ?>" />
			<?php _e('day(s)','dbem'); ?>
		...
		<div class="text-info"><i class="fa fa-info"></i> <?php esc_html_e('This is the definite date after which bookings will be closed for this event, regardless of individual ticket settings above. Default value will be the event start date.','dbem'); ?></div>
	</p>
	<?php endif; ?>
	</div>
	<?php
