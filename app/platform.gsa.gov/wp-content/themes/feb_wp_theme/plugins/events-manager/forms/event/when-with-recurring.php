...
<div id="em-form-with-recurrence" class="event-form-with-recurrence event-form-when subform-group">
	<div class="form-group">
         <input type="checkbox" id="em-recurrence-checkbox" name="recurring" value="1" <?php if($EM_Event->is_recurring()) echo 'checked' ?> />  <?php _e('This is a recurring event.', 'dbem'); ?>
</div>
	<div class="em-date-range form-group">
		<label class="em-recurring-text"><?php _e ( 'From ', 'dbem' ); ?></label><label class="em-event-text"><?php _e ( 'From ', 'dbem' ); ?></label><input class="em-date-start em-date-input-loc form-control" type="text" /><input class="em-date-input form-control" type="hidden" name="event_start_date" value="<?php echo $EM_Event->event_start_date ?>" />
		<label class="em-recurring-text"><?php _e('to','dbem'); ?></label><label class="em-event-text"><?php _e('to','dbem'); ?></label>
		<input class="em-date-end em-date-input-loc form-control" type="text" /><input class="em-date-input form-control" type="hidden" name="event_end_date" value="<?php echo $EM_Event->event_end_date ?>" />
	</div>
	<div class="form-group">
		<label class="em-recurring-text"><?php _e('Events starts from','dbem'); ?></label><label class="em-event-text"><?php _e('Event starts at','dbem'); ?></label><input id="start-time" class="em-time-input em-time-start form-control" type="text" size="8" maxlength="8" name="event_start_time" value="<?php echo date( $hours_format, $EM_Event->start ); ?>" /><label><?php _e('to','dbem'); ?></label><input id="end-time" class="em-time-input em-time-end form-control" type="text" size="8" maxlength="8" name="event_end_time" value="<?php echo date( $hours_format, $EM_Event->end ); ?>" />
		 <input type="checkbox" class="em-time-allday" name="event_all_day" id="em-time-all-day" value="1" <?php if(!empty($EM_Event->event_all_day)) echo 'checked="checked"'; ?> />  <?php _e('All day','dbem'); ?>
	</div>
