...
function challenge_signup_schema($oldVal, $newVal){
...
add_action( 'update_option_challenge_general_settings', 'challenge_signup_schema', 10, 2 );


//function to add new tab in the default WP userlisting with all the users who didn't confirm their account yet
function challenge_add_pending_users_header_script(){
?>
	<script type="text/javascript">	
		jQuery(document).ready(function() {
			jQuery.post( ajaxurl ,  { action:"challenge_get_unconfirmed_email_number"}, function(response) {
		...
		function confirmECAction(URL, todo, userID, actionText) {
			...
			if (confirm(actionText)) {
				jQuery.post( ajaxurl ,  { action:"challenge_handle_email_confirmation_cases", URL:URL, todo:todo, userID:userID}, function(response) {
					if (response == 'ok'){
						window.location=URL;
					}else{
						alert(response);
...
function challenge_get_unconfirmed_email_number(){
	global $wpdb;
...
function challenge_handle_email_confirmation_cases() {
	global $current_user;
	global $wpdb;
	...
	if (current_user_can('delete_users'))
		if (($todo != '') && ($userID != '')){
			...
			if ($todo == 'delete'){
				...
				while ($row=mysql_fetch_row($result)){
					if ((string)$iterator === $userID){
						$ret = challenge_manual_activate_signup($row[8]);
						die($ret);	
					}
					$iterator++;
				}
...
add_action('wpmu_activate_user','challenge_add_meta_to_user_on_activation',10,3);

//function that adds AP user meta after signup autentification and it is also used when editing the user uprofile
function challenge_add_meta_to_user_on_activation($user_id, $password, $meta){
	...
	$challenge_premium = challenge_PLUGIN_DIR . '/premium/functions/';
	if (file_exists ( $challenge_premium.'extra.fields.php' )){
		$challengeFetchArray = get_option('challenge_custom_fields');
		foreach ( $challengeFetchArray as $key => $value){
				...
				case "avatar":{
					if( !empty($meta[$value['item_type'].$value['id']] ) ){
						...
						if ($renamedVar){
							...
							challenge_resize_avatar($user_id);
						}
					}				
					break;
				}
 ...
 *
 * Filter 'challenge_signup_user_notification_filter' to bypass this function or
 * replace it with your own notification behavior.
 *
 * Filter 'challenge_signup_user_notification_email' and
 * 'challenge_signup_user_notification_subject' to change the content
 * and subject line of the email sent to newly registered users.
 *
 * @param string $user The user's login name.
 * @param string $user_email The user's email address.
 * @param array $meta By default, an empty array.
 * @param string $key The activation key created in challenge_signup_user()
 * @return bool
 */
function challenge_signup_user_notification($user, $user_email, $key, $meta = '') {
	...
	$subject = sprintf(apply_filters( 'challenge_signup_user_notification_subject', __( '[%1$s] Activate %2$s', 'profilebuilder'), $user, $user_email, $key, $meta ), $from_name, $user);
	$message = sprintf(apply_filters( 'challenge_signup_user_notification_email', __( "To activate your user, please click the following link:\n\n%s%s%s\n\nAfter you activate, you will receive *another email* with your login.\n\n", "profilebuilder" ),$user, $user_email, $key, $meta), '<a href="'.$siteURL.'">', $siteURL, '</a>.');
 ...
 */
function challenge_manual_activate_signup($key) {
	global $wpdb;
	...
	if ( !empty( $signup ) && !$signup->active ){
		...
		if ( ! $user_id )
			$user_id = challenge_create_user($user_login, $password, $user_email);
		else
		...
		elseif ( isset( $user_already_exists ) )
			...
			if ( is_multisite() )
				...
				challenge_add_meta_to_user_on_activation($user_id, '', $meta);
				...
				challenge_notify_user_registration_email($bloginfo, $user_login, $user_email, 'sending', $password, $challenge_generalSettings['adminApproval']);
				
				do_action('challenge_activate_user', $user_id, $password, $meta);
 ...
 */
function challenge_create_user( $user_name, $password, $email) {
	...
	do_action( 'challenge_new_user', $user_id );
...
function challenge_notify_user_registration_email($bloginfo, $user_name, $email, $send_credentials_via_email, $passw1, $adminApproval){
	...
	if (isset($send_credentials_via_email) && ($send_credentials_via_email == 'sending')){
		...
		$registerFilterArray['userMessageContent'] = sprintf(__( 'Welcome to %1$s!<br/><br/> Your username is: %2$s.', 'profilebuilder'), $registerFilterArray['userMessageFrom'], $user_name, $passw1);
		if ($adminApproval == 'yes')
...
add_action( 'wp_ajax_challenge_get_unconfirmed_email_number', 'challenge_get_unconfirmed_email_number' );	
add_action( 'wp_ajax_challenge_handle_email_confirmation_cases', 'challenge_handle_email_confirmation_cases' );

if (is_multisite()){
			
	if (strpos($_SERVER['SCRIPT_NAME'], 'users.php')){  //global $pagenow doesn't seem to work
		add_action( 'admin_head', 'challenge_add_pending_users_header_script' );

	}
	if (file_exists ( challenge_PLUGIN_DIR . '/premium/functions/admin.approval.php' ))
		add_action( 'user_register', 'challenge_update_user_status_on_admin_registration' );
	
}else{
	$challenge_generalSettings = get_option('challenge_general_settings', 'not_found');
	if($challenge_generalSettings != 'not_found')
		if(!empty($challenge_generalSettings['emailConfirmation']) && ($challenge_generalSettings['emailConfirmation'] == 'yes')){
			global $pagenow;
