...
if(!function_exists('challege_curpageurl_password_recovery')){
    function challege_curpageurl_password_recovery() {
		$pageURL = 'http';
...
if(!function_exists('challege_curpageurl_password_recovery2')){
    function challege_curpageurl_password_recovery2($user_login, $id) {
		...
		if ( empty($key) ) {
			...
			do_action('challege_retrieve_password_key', $user_login, $key);
...
if(!function_exists('challege_curpageurl_password_recovery3')){
    function challege_curpageurl_password_recovery3() {
		$pageURL = 'http';
		if ((isset($_SERVER["HTTPS"])) && ($_SERVER["HTTPS"] == "on")) {
			$pageURL .= "s";
		}
...
function challege_check_for_unapproved_user($data, $what){
	$retArray = array( 0 => '', 1 => '');
	$retMessage = '';
	$messageNo = '';
	
	$challege_generalSettings = get_option('challege_general_settings');
	
	if($challege_generalSettings['adminApproval'] == 'yes'){
		...
		if (wp_get_object_terms( $userID, 'user_status' )){
			...
			$retMessage = apply_filters('challege_recover_password_unapporved_user', $retMessage);
...
function challenge_front_end_password_recovery(){
	...
	if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'recover_password' && wp_verify_nonce($_POST['password_recovery_nonce_field'],'verify_true_password_recovery') ) {
		...
		if (is_email($postedData)){
			if (email_exists($postedData)){
				$retVal = challege_check_for_unapproved_user($postedData, 'user_email');
				if ($retVal[0] != ''){
					$message = $retVal[0];
					$messageNo = $retVal [1];
					...
					$recoverPasswordFilterArray['sentMessage1'] = apply_filters('challege_recover_password_sent_message1', $recoverPasswordFilterArray['sentMessage1'], $postedData);
					...
					$recoverPasswordFilterArray['userMailMessage1']  = sprintf(__('Someone requested that the password be reset for the following account: <b>%1$s</b><br/>If this was a mistake, just ignore this email and nothing will happen.<br/>To reset your password, visit the following link: %2$s', 'profilebuilder'), $requestedUserLogin, '<a href="'.challege_curpageurl_password_recovery2($requestedUserLogin, $requestedUserID).'">'.challege_curpageurl_password_recovery2($requestedUserLogin, $requestedUserID).'</a>');
					$recoverPasswordFilterArray['userMailMessage1']  = apply_filters('challege_recover_password_message_content_sent_to_user1', $recoverPasswordFilterArray['userMailMessage1'], $requestedUserID, $requestedUserLogin);
					...
					$recoverPasswordFilterArray['userMailMessageTitle1'] = apply_filters('challege_recover_password_message_title_sent_to_user1', $recoverPasswordFilterArray['userMailMessageTitle1']);
					...
					if (trim($recoverPasswordFilterArray['userMailMessageTitle1']) != ''){
					    $headers[] = 'From: Challenge.gov <challenge@gsa.gov>' . "\r\n";
						$sent = wp_mail($requestedUserEmail, $recoverPasswordFilterArray['userMailMessageTitle1'], $recoverPasswordFilterArray['userMailMessage1'],$headers);
						if ($sent === false){
							...
							$recoverPasswordFilterArray['sentMessageCouldntSendMessage'] = apply_filters('challege_recover_password_sent_message_error_sending', $recoverPasswordFilterArray['sentMessageCouldntSendMessage']);
							$messageNo = '5';
				...
				$recoverPasswordFilterArray['sentMessage2'] = apply_filters('challege_recover_password_sent_message2', $recoverPasswordFilterArray['sentMessage2']);
				$messageNo = '2';
				$message = $recoverPasswordFilterArray['sentMessage2'];
			}
		}elseif (!is_email($postedData)){
			if (username_exists($postedData)){
				$retVal = challege_check_for_unapproved_user($postedData, 'user_login');
				if ($retVal[0] != ''){
					$message = $retVal[0];
					$messageNo = $retVal [1];
					...
					$recoverPasswordFilterArray['sentMessage3'] = apply_filters('challege_recover_password_sent_message3', $recoverPasswordFilterArray['sentMessage3']);
					$messageNo = '3';
					...
					//send primary email message
					$recoverPasswordFilterArray['userMailMessage1']  = sprintf(__('You have requested that the password be reset for the following account: <b>%1$s</b><br/>If this was a mistake, just ignore this email.<br/>To reset your password, visit the following link:%2$s', 'profilebuilder'), $requestedUserLogin, '<a href="'.challege_curpageurl_password_recovery2($requestedUserLogin, $requestedUserID).'">'.challege_curpageurl_password_recovery2($requestedUserLogin, $requestedUserID).'</a>');
					$recoverPasswordFilterArray['userMailMessage1']  = apply_filters('challege_recover_password_message_content_sent_to_user1', $recoverPasswordFilterArray['userMailMessage1'], $requestedUserID, $requestedUserLogin);
				...
				$recoverPasswordFilterArray['sentMessage4'] = apply_filters('challege_recover_password_sent_message4', $recoverPasswordFilterArray['sentMessage4']);
				$messageNo = '4';
	...
	elseif ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action2'] ) && $_POST['action2'] == 'recover_password2' && wp_verify_nonce($_POST['password_recovery_nonce_field2'],'verify_true_password_recovery2') ) {
		if (($_POST['passw1'] == $_POST['passw2']) && (!empty($_POST['passw1']) && !empty($_POST['passw2']))){
			...
			do_action('challege_password_reset', $userID, $new_pass);
			...
			$recoverPasswordFilterArray['userMailMessage2']  = sprintf(__('You have successfully reset your Challenge account password!', 'profilebuilder'));
			$recoverPasswordFilterArray['userMailMessage2']  = apply_filters('challege_recover_password_message_content_sent_to_user2', $recoverPasswordFilterArray['userMailMessage2'], $loginName);
			...
			$recoverPasswordFilterArray['userMailMessageTitle2'] = apply_filters('challege_recover_password_message_title_sent_to_user2', $recoverPasswordFilterArray['userMailMessageTitle2']);
			$headers[] = 'From: Challenge.gov <challenge@gsa.gov>' . "\r\n";
			//we add this filter to enable html encoding
			add_filter('wp_mail_content_type',create_function('', 'return "text/html"; '));
			//send mail to the user notifying him of the reset request
			if (trim($recoverPasswordFilterArray['userMailMessageTitle2']) != '')
				wp_mail($user_info->user_email, $recoverPasswordFilterArray['userMailMessageTitle2'], $recoverPasswordFilterArray['userMailMessage2'],$headers);
			...
			$recoverPasswordFilterArray['adminMailMessage'] = apply_filters('challege_recover_password_message_content_sent_to_admin', $recoverPasswordFilterArray['adminMailMessage']);
			...
			$recoverPasswordFilterArray['adminMailMessageTitle'] = apply_filters('challege_recover_password_message_title_sent_to_admin', $recoverPasswordFilterArray['adminMailMessageTitle']);
...
?>
	...
	<div class="challege_holder" id="challege_recover_password">

<?php
			/* use this action hook to add extra content before the password recovery form. */
			do_action( 'challege_before_recover_password_fields' );
				...
				if ( !empty( $user ) ){
					...
					if (isset($_GET['finalAction']) && ($_GET['finalAction'] == 'yes')){
						if ($messageNo2 == '2'){
							...
							$recoverPasswordFilterArray['passwordChangedMessage2'] = apply_filters ('challege_recover_password_password_changed_message2', $recoverPasswordFilterArray['passwordChangedMessage2'], $message2);
							echo $recoverPasswordFilterArray['passwordChangedMessage2'];
?>
							<form enctype="multipart/form-data" method="post" id="recover_password2" class="user-forms" action="<?php echo $url=challege_curpageurl_password_recovery3();?>">
<?php
								...
								$recoverPasswordFilterArray['inputPassword'] = apply_filters('challege_recover_password_input', $recoverPasswordFilterArray['inputPassword'], $_POST['passw1'], $_POST['passw2'], $user->ID);
...
<?php
							...
							$recoverPasswordFilterArray['passwordChangedMessage1'] = apply_filters ('challege_recover_password_password_changed_message1', $recoverPasswordFilterArray['passwordChangedMessage1'], $message2);
...
<?php
					...
					if ($messageNo2 == '1'){
						...
						echo $recoverPasswordFilterArray['invalidKeyMessage'] = apply_filters('challege_recover_password_invalid_key_message', $recoverPasswordFilterArray['invalidKeyMessage']);
					}
				}
				...
				if (($messageNo == '') || ($messageNo == '2') || ($messageNo == '4')){
					...
					$recoverPasswordFilterArray['messageDisplay1'] = apply_filters('challege_recover_password_displayed_message1', $recoverPasswordFilterArray['messageDisplay1']);
					echo $recoverPasswordFilterArray['messageDisplay1'];
					
					echo '<form enctype="multipart/form-data" method="post" id="recover_password" class="user-forms" action="'.$address = challege_curpageurl_password_recovery().'">';
						...
						echo $recoverPasswordFilterArray['notification'] = apply_filters('challege_recover_password_message1', $recoverPasswordFilterArray['notification']);
						...
						$recoverPasswordFilterArray['input'] = apply_filters('challege_recover_password_input', $recoverPasswordFilterArray['input'], trim($username_email));
	...
	<?php
					...
					$recoverPasswordFilterArray['messageDisplay2'] = apply_filters('challege_recover_password_displayed_message2', $recoverPasswordFilterArray['messageDisplay2']);
			...
			do_action( 'challege_after_recover_password_fields' );
?>
	</div>
...
<?php
	...
	$recoverPasswordFilterArray = apply_filters('challege_recover_password', $recoverPasswordFilterArray);
