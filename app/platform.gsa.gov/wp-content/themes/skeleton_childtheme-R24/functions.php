...
function st_header_scripts() {
  ...
  $javascripts .= wp_enqueue_script('custom',get_bloginfo('template_url') ."/javascripts/app.js",array('jquery'),'1.2.3',true);
    ...
    $javascripts .= wp_enqueue_script('formalize',get_bloginfo('template_url') ."/javascripts/jquery.scrollTo.js",
        array('jquery'),'1.2.3',true);
...
function st_widgets_init() {
	...
	register_sidebar(array(
		...
		'before_widget' => '<div class="sidebar-block-wrap">',
		'after_widget' => '</div>',
		'before_title' => '<h3>',
		'after_title' => '</h3>'
	) );
