...
class EM_Coupon extends EM_Object {
	...
	function get_count( $refresh = false ){
		...
		if( $refresh ){
			...
			$this->count = $wpdb->get_var('SELECT COUNT(*) FROM '.EM_BOOKINGS_TABLE." WHERE booking_meta LIKE '%{$coupon_search}%'");
		}elseif( !$this->count ){
	...
	function recount(){
		...
		return $wpdb->update(EM_META_TABLE, array('meta_value' => $count), array('meta_key' => 'coupon-count', 'object_id' => $this->coupon_id));
	}
